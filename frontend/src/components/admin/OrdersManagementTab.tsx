...
import { useGetAllOrders, useUpdateOrderStatus, useOrderStatusHistory } from '../../hooks/useQueries';
import { OrderStatus, BookOrder } from '../../backend';
...
function OrderRow({ order }: { order: BookOrder }) {
  ...
  const { data: statusHistory } = useOrderStatusHistory(expanded ? order.orderId : -1);
    ...
    <div className="bg-card border border-border rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-4 flex-wrap">
          <div>
            <p className="font-semibold text-foreground text-sm">Order #{order.orderId}</p>
            <p className="text-xs text-muted-foreground">{formatDate(order.createdAt)}</p>
          </div>
          <div>
            <p className="text-sm font-medium text-foreground">{order.buyerName}</p>
            <p className="text-xs text-muted-foreground">{order.deliveryInfo.companyName}</p>
          </div>
          {getStatusBadge(order.status)}
        </div>
        <div className="flex items-center gap-3">
          <span className="font-bold text-primary">{formatINR(order.totalAmountINR)}</span>
          {expanded ? (
      ...
      {expanded && (
        <div className="border-t border-border p-4 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <h4 className="font-semibold text-foreground text-sm mb-2">Customer Details</h4>
              <div className="space-y-1.5 text-sm">
                <p><span className="text-muted-foreground">Name: </span><span className="font-medium">{order.buyerName}</span></p>
                <p><span className="text-muted-foreground">Contact: </span><span className="font-medium">{order.buyerContact}</span></p>
              </div>
            </div>
            <div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between font-bold text-sm pt-1">
                <span>Total</span>
                <span className="text-primary">{formatINR(order.totalAmountINR)}</span>
              </div>
            </div>
          </div>
          ...
          {/* Status Update */}
          <div>
            <h4 className="font-semibold text-foreground text-sm mb-2">Update Status</h4>
            <div className="flex items-center gap-3">
              <Select
          ...
          {statusHistory && statusHistory.length > 0 && (
            <div>
              <h4 className="font-semibold text-foreground text-sm mb-2">Status History</h4>
              <div className="space-y-2">
